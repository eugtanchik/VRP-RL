...
class RLAgent(object):
                ...
                with tf.compat.v1.variable_scope("Linear"):
                    v = tf.squeeze(
                        tf.compat.v1.layers.dense(
                            tf.compat.v1.layers.dense(hy, args['hidden_dim'], tf.nn.relu, name='L1'), 1, name='L2'
                        ),
                        1
                    )
    ...
    def build_train_step(self):
        ...
        # clip gradients
        clip_actor_gra_and_var = [
            (tf.clip_by_norm(grad, args['max_grad_norm']), var) for grad, var in actor_gra_and_var
        ]

        clip_critic_gra_and_var = [
            (tf.clip_by_norm(grad, args['max_grad_norm']), var) for grad, var in critic_gra_and_var
        ]
    ...
    def evaluate_single(self, eval_type='greedy'):
        ...
        self.prt.print_out("Finished evaluation with %d steps in %s." % (
            step, time.strftime("%H:%M:%S", time.gmtime(end_time)))
        )
